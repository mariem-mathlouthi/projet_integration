<template>
  ...
  >
    ...
    <div class="flex flex-col gap-4 py-4 lg:flex-row">
      ...
      >
        <div v-if="imageBase64 == null" class="h-16 w-16 rounded-full">
...
export default {
  ...
  methods: {
    ...
    async saveChanges() {
      ...
      const modifyStudent = await axios.post("http://localhost:8000/api/modifyStudent", myjson),
        uploadImg = await axios.post("http://localhost:8000/api/uploadfile", myjson);

      return Promise.all([modifyStudent, uploadImg])
